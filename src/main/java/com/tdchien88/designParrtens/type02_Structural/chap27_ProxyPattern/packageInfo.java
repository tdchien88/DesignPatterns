 ...
 * It is used:
 * 
 * _ It can be used in Virtual Proxy scenario---Consider a situation where there
 * is multiple database call to extract huge size image. Since this is an
 * expensive operation so here we can use the proxy pattern which would create
 * multiple proxies and point to the huge size memory consuming object for
 * further processing. The real object gets created only when a client first
 * requests/accesses the object and after that we can just refer to the proxy to
 * reuse the object. This avoids duplication of the object and hence saving
 * memory.
 * 
 * _ It can be used in Protective Proxy scenario---It acts as an authorization
 ...
 * 
 * _ It can be used in Remote Proxy scenario---A remote proxy can be thought
 * about the stub in the RPC call. The remote proxy provides a local
 ...
 * 
 * _ It can be used in Smart Proxy scenario---A smart proxy provides additional
 * layer of security by interposing specific actions when the object is
 * accessed. For example, to check whether the real object is locked or not
